...
@section('header_title', 'Communications Management')
@section('header_subtitle', 'Monitor campaigns, engagement, and delivery performance across all outreach channels.')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-slate-200/80 shadow-2xs overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h3 class="text-base font-bold text-slate-900">Recent Communications</h3>
                <p class="text-[11px] text-slate-500">Overview of latest messages sent across email and CRM workflows.</p>
            </div>
                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @foreach($communications as $message)
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            <td class="py-3 px-4 font-mono font-bold text-sky-700">{{ $message['id'] }}</td>
                            <td class="py-3 px-4 font-semibold text-slate-900">{{ $message['subject'] }}</td>
                            <td class="py-3 px-4"><span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-700">{{ $message['type'] }}</span></td>
                            <td class="py-3 px-4 text-slate-600">{{ $message['audience'] }}</td>
                            <td class="py-3 px-4">{{ $message['channel'] }}</td>
                            <td class="py-3 px-4 text-slate-600">{{ $message['sentOn'] }}</td>
                            <td class="py-3 px-4"><span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold {{ $message['status_class'] }}">{{ $message['status'] }}</span></td>
                            <td class="py-3 px-4 text-right font-bold text-slate-800">{{ $message['openRate'] }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
